"""Stolen and then adapted from: https://github.com/kenshohara/3D-ResNets-PyTorch"""
...
class ResNet3d(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv3d(num_channels_in, self.in_planes, kernel_size=(3, 3, 3), padding="same", bias=False)
        self.bn1 = nn.BatchNorm3d(self.in_planes)
        self.relu = nn.ReLU()
        self.maxpool = nn.MaxPool3d(kernel_size=3, stride=1)
        self.layer1 = self._make_layer(block, block_inplanes[0], nums_layers[0], shortcut_type)
        self.layer2 = self._make_layer(block, block_inplanes[1], nums_layers[1], shortcut_type, stride=2)
        self.layer3 = self._make_layer(block, block_inplanes[2], nums_layers[2], shortcut_type, stride=2)
        self.layer4 = self._make_layer(block, block_inplanes[3], nums_layers[3], shortcut_type, stride=2)

        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        self.fc = nn.Linear(block_inplanes[3] * block.expansion, num_channels_out)
